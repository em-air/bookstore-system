...
export default async function handler(req: VercelRequest,res: VercelResponse){
  ...
  if(req.method==='PATCH'){
    ...
    const status=body.status; if(!['approved','denied'].includes(status)) return res.status(400).json({message:'Invalid status'});
    try{ const refund=await storage.updateRefundStatus(id,status); if(!refund) return res.status(404).json({message:'Refund not found'}); return res.status(200).json(refund);}catch{ return res.status(500).json({message:'Failed to update refund'});}
  }
