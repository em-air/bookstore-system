...
export default async function handler(req: VercelRequest,res: VercelResponse){
  ...
  if(req.method==='GET'){ try{ const orders=await storage.getAllOrders(); return res.status(200).json(orders);}catch{ return res.status(500).json({message:'Failed to fetch orders'});} }
